function plotsrcclusters(stat,varargin)
...
if stat.cfg.tail >= 0
    ...
    figfile = sprintf('figures/%s_%s_%s-%s-%s_pos',stat.statmode,num2str(stat.subjinfo),stat.condlist{1},stat.condlist{2},stat.scoutname);
    ...
    %ylim = ylim*2;
    ylim = param.ylim;
    set(gca,'YLim',ylim,'XLim',[stat.diffcond.time(1) stat.diffcond.time(end)]-stat.timeshift,...
    'YTick',param.ylim(1):40:param.ylim(2),'XTick',stat.diffcond.time(1)-stat.timeshift:0.2:stat.diffcond.time(end)-stat.timeshift,...
    ...
    if ~isempty(posclustidx)
        ...
        title(sprintf('Cluster @ %.3fs (t = %.2f, p = %.3f)', stat.diffcond.time(maxtime)-stat.timeshift, stat.posclusters(posclustidx).clusterstat, stat.posclusters(posclustidx).prob),...
            'FontName',fontname,'FontSize',fontsize);
    else
...
if stat.cfg.tail <= 0
    ...
    figfile = sprintf('figures/%s_%s_%s-%s-%s_neg',stat.statmode,num2str(stat.subjinfo),stat.condlist{1},stat.condlist{2},stat.scoutname);
    ...
    if ~isempty(negclustidx)
        ...
        clustwinidx = find(clust_t);
        line([stat.time(clustwinidx(1)) stat.time(clustwinidx(end))]-stat.timeshift,[0 0],'Color','blue','LineWidth',8);
        title(sprintf('Cluster @ %.3fs (t = %.2f, p = %.3f)', stat.diffcond.time(mintime)-stat.timeshift, stat.negclusters(negclustidx).clusterstat,stat.negclusters(negclustidx).prob),...
            'FontName',fontname,'FontSize',fontsize);
    else
        title(' ','FontName',fontname,'FontSize',fontsize);
    end
