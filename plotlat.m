function plotlat(stat,statall,varargin)
...
if ~isempty(posclustidx)
    ...
    for p = 1:length(posclustidx)
        ...
        %remove baseline
        statall.diffcond.cond1avg = rmbase(statall.diffcond.cond1avg,[],find(statall.diffcond.time-statall.timeshift >= -0.2 & statall.diffcond.time-statall.timeshift <= 0));
        statall.diffcond.cond2avg = rmbase(statall.diffcond.cond2avg,[],find(statall.diffcond.time-statall.timeshift >= -0.2 & statall.diffcond.time-statall.timeshift <= 0));
        ...
        timevals = statall.diffcond.time-statall.timeshift;
        areawin = [stat.cfg.latency(1) stat.diffcond.cond1avg(maxchan)] - stat.timeshift;
        timeidx = timevals >= areawin(1) & timevals <= areawin(2);
        area(timevals(timeidx),statall.diffcond.cond1avg(maxchan,timeidx),'FaceColor',colororder(1,:),'LineStyle','none');

        timevals = statall.diffcond.time-statall.timeshift;
        areawin = [stat.cfg.latency(1) stat.diffcond.cond2avg(maxchan)] - stat.timeshift;
        timeidx = timevals >= areawin(1) & timevals <= areawin(2);
        area(timevals(timeidx),statall.diffcond.cond2avg(maxchan,timeidx),'FaceColor',colororder(2,:),'LineStyle','none');

%         line([stat.diffcond.cond1avg(maxchan) stat.diffcond.cond1avg(maxchan)]-stat.timeshift,ylim,'LineWidth',linewidth,'LineStyle','--','Color',colororder(1,:));
%         line([stat.diffcond.cond2avg(maxchan) stat.diffcond.cond2avg(maxchan)]-stat.timeshift,ylim,'LineWidth',linewidth,'LineStyle','--','Color',colororder(2,:));
        ...
        title(sprintf('Latency diff. = %.3f sec (t = %.2f, p = %.3f) ',stat.diffcond.cond1avg(maxchan)-stat.diffcond.cond2avg(maxchan),...
...
if ~isempty(negclustidx)
    ...
    for p = 1:length(negclustidx)
        ...
        line([stat.diffcond.cond1avg(minchan) stat.diffcond.cond1avg(minchan)]-stat.timeshift,ylim,'LineWidth',linewidth,'LineStyle','--','Color',colororder(1,:));
        line([stat.diffcond.cond2avg(minchan) stat.diffcond.cond2avg(minchan)]-stat.timeshift,ylim,'LineWidth',linewidth,'LineStyle','--','Color',colororder(2,:));
        xlabel('Time relative to 5th tone (sec) ','FontName',fontname,'FontSize',fontsize);
        ylabel('Amplitude (uV)','FontName',fontname,'FontSize',fontsize);
        title(sprintf('Latency diff. = %.3f sec (t = %.2f, p = %.3f) ',stat.diffcond.cond1avg(minchan)-stat.diffcond.cond2avg(minchan),...
